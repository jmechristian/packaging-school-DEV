...
const WiredCourseCard = ({ id }) => {
  ...
  useEffect(() => {
    id && setIsLesson(allCourses && allCourses.filter((c) => c.id === id));
  }, [id, allCourses]);

  return isLesson && isLesson[0] ? (
    <motion.div
      ...
      onClick={() => router.push(`/courses/${isLesson[0].slug}`)}
    >
      ...
      <motion.div
        ...
        style={{
          backgroundImage: `url(${isLesson[0].seoImage})`,
        }}
      ></motion.div>
      ...
      {isPlaying ? (
        <motion.div className='aspect-[16/9] w-full h-auto relative z-[2] flex items-center justify-center bg-black transition-opacity ease-in'>
          <VideoPlayer
            videoEmbedLink={isLesson[0].preview}
            light={false}
            playing={true}
          />
        </motion.div>
        ...
        <motion.div
          ...
          onClick={isLesson[0].preview ? () => setIsPlaying(true) : () => {}}
        >
          {isLesson[0].preview && (
        ...
        <motion.div className='mt-4 h-full min-h-[150px] bg-white/70 backdrop-blur shadow-lg rounded-b-lg mx-2 rounded-lg mb-4 relative z-[2] flex flex-col flex-1 '>
          <motion.div className='flex flex-col gap-3 h-full pt-4'>
            ...
            <motion.div className='text-sm leading-tight px-3 line-clamp-4 text-neutral-600'>
              {isLesson[0].subheadline}
            </motion.div>
          </motion.div>
        </motion.div>
        <motion.div className='bg-black w-full rounded-b-lg z-10 relative text-center cursor-pointer'>
          <div className='text-white font-bold px-6 py-3'>View Course</div>
        </motion.div>
      </motion.div>
    </motion.div>
  ) : (
